...
같은 "장중 마이너스 / 오버나이트 플러스" 가설이 미국에도 통하는지는 아직 실데이터로 확인 전이다.
국내는 KRX 기관/외국인 순매수라는 무료 신호가 있었지만 미국은 그런 데이터가 없어서
"당일 등락률 상위 + 거래량" 을 대체 신호로 쓴다. 자세한 내용과 검증 순서는 `deploy/README.md` 를 본다.

```bash
python -m overseas.fetch_data && python -m overseas.backtest --top 20 --min-chg 0.03
```

이 숫자를 먼저 보고 국내처럼 몇 주 페이퍼를 거친 뒤에 판단한다.
